        ...
        function buildHevyWorkoutPayload() {
            ...
            const exercises = dayExercises.map(exercise => {
                ...
                const lastSetNote = (usesRIR && lastSetValue !== undefined)
                    ...
                    : '';
                const trimmedNote = lastSetNote.trim();
                ...
                if (trimmedNote.length > 0) {
                    exercisePayload.notes = trimmedNote;
                }
            ...
            return normalizeHevyWorkoutPayload(payload);
        }

        function normalizeHevyWorkoutPayload(payload) {
            const title = String(payload?.title || '').trim();
            if (!title) {
                throw new Error('Workout title is required for Hevy export.');
            }
            const exercises = Array.isArray(payload?.exercises) ? payload.exercises : [];
            if (exercises.length === 0) {
                throw new Error('Hevy export requires at least one exercise.');
            }

            const normalizedExercises = exercises.map(exercise => {
                const sets = Array.isArray(exercise?.sets) ? exercise.sets : [];
                if (sets.length === 0) {
                    throw new Error('Hevy export requires at least one set per exercise.');
                }
                ...
                if (typeof exercise?.notes === 'string' && exercise.notes.trim().length > 0) {
                    normalizedExercise.notes = exercise.notes.trim();
                }
